...
def check_recompute_necessary(inputs):
    if not any(input_.stop_gradient == False for input_ in inputs
        ...
        logging.warn(
            "[Recompute]: None of the inputs to current recompute block need grad, therefore there is NO need to recompute this block in backward !"
        )
...
def recompute(function, *args, **kwargs):
    ...
    Args:
        function: layer of sequence of layers that describes part of forward pass of the model 
        preserve_rng_state(bool, optional, default=True):  if retore the RNG state of forward in backward 
